...
void position_7(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
	...
	Y_new=-X_vision
	 */
	Position_7(0)=msg->pose.position.y;
	Position_7(1)=-msg->pose.position.x;
	Position_7(2)=msg->pose.position.z;
}
...
void Target_publisher()
{
	...
	}else if (Target_selector==142)
	{
		...
		msg_p1.position.z=Position_14_target_2(2)+1.35;
		msg_p1.orientation.x=q_orien.x();
		msg_p1.orientation.y=q_orien.y();
		msg_p1.orientation.z=q_orien.z();
		msg_p1.orientation.w=q_orien.w();
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		Quaterniond q1(mat);
		Object_left.orientation.x=q1.x();
		Object_left.orientation.y=q1.y();
		Object_left.orientation.z=q1.z();
		Object_left.orientation.w=q1.w();
		...
		mat.block(0,0,3,1)=Orientation_grasp_x[1];
		mat.block(0,1,3,1)=Orientation_grasp_y[1];
		mat.block(0,2,3,1)=Orientation_grasp_z[1];
		Quaterniond q2(mat);
		Object_right.orientation.x=q2.x();
		Object_right.orientation.y=q2.y();
		Object_right.orientation.z=q2.z();
		Object_right.orientation.w=q2.w();
