...
void task_allocation::Initialize_motion_predication_only(int N_robots,int N_grabbing_pos, double dt, int N_state,ENUM_State_of_prediction Object_motion)
{
...
void task_allocation::Set_the_object_state(VectorXd X,VectorXd DX)
{
...
void task_allocation::Set_the_object_state_for_prediction(VectorXd X,VectorXd X_filtered, double time)
{
	/* Setting the current state of the object
	 * X is the state of the object with respect to the world-frame
	 * DX is the  derivative of the state of the object		*/
...
void task_allocation::Set_index_of_grabbing_posititon_(int index_of_robot,int indext_of_grabbing_pos)
{
	Robots_[index_of_robot].index_of_grabbing_posititon_=indext_of_grabbing_pos;
}

void task_allocation::Set_the_grabbing_state(int index,VectorXd X_G)
{
...
void task_allocation::Set_the_grabbing_state(int index,VectorXd X_G,VectorXd X_O)
{
	/* Setting the grabbing states on the object with respect to the universal frame (the same frame as the object and etc...
	 * X is the state of index th grabbing states with respect to the world-frame
	 * Notice: you just need to set it once! and you need to set it exactly after the object state is set!		*/
...
void task_allocation::Set_pos_of_grabbing_posititon_for_object_(bool catching_pos_is_found,double likelihood, Vector3d X_I_C)
{
...
void task_allocation::Set_index_of_grabbing_posititon_(int index_of_robot, int index_of_grabbing, Vector3d X_I_C)
{
...
void task_allocation::Initialize_the_virtual_object()
{
...
void task_allocation::calculate_coordination_allocation()
{
	...
	for (int i=0;i<N_robots_;i++)
	{
		...
		/*		if (Object_.X_O_INTERCEPT_.block(0,0,3,1).norm()==0)
		{
			handle(0,0)=0.1;
		}
		else
		{
		//	handle=-(Object_.DX_O_.block(0,0,3,1).transpose()*Object_.X_O_INTERCEPT_.block(0,0,3,1).normalized());
			handle=Object_.DX_O_(0);
						cout<<"Object_.X_O_INTERCEPT_.block(0,0,3,1) "<<Object_.X_O_INTERCEPT_<<endl;
			cout<<"Object_.DX_O_.block(0,0,3,1) "<<Object_.DX_O_<<endl;
		}
		//MatrixXd handle_M=-((Object_.DX_O_.block(0,0,3,1)-Vobject_.DX_V_INTERCEPT_.block(0,0,3,1)).transpose()*(Object_.X_O_.block(0,0,3,1)-Vobject_.X_V_.block(0,0,3,1)).normalized());
		if (fabs(handle(0,0))<0.001)
		{
			handle(0,0)=0.0001;
		}
		Robots_[i].M[0]=handle(0,0);*/
		if ((Object_.Max_liklihood==10)&&(Object_.DX_O_(0)<0))
		{
			Robots_[i].M[0]=0.1;
		}
		else
		...
		//	handle=-(Object_.DX_O_.block(0,0,3,1).transpose()*Object_.X_O_INTERCEPT_.block(0,0,3,1))/(0.0001+Object_.X_O_INTERCEPT_.block(0,0,3,1).norm()*Object_.X_O_INTERCEPT_.block(0,0,3,1).norm());
	//		Robots_[i].M[0]=handle(0,0);
		}

		/*		if ((Object_.X_O_.block(0,0,3,1)-Vobject_.X_V_.block(0,0,3,1)).norm()<0.1)
		{
			Robots_[i].M[1]=fabs(sgn(Robots_[i].index_of_grabbing_posititon_+2))*fabs(Object_.Max_liklihood)*min(sgn(Object_.Max_liklihood),sgn(Robots_[i].index_of_grabbing_posititon_+2));
			Robots_[i].Dtau_=Robots_[i].M[1]/(0.1);
		}
		else
		{*/
		...
		//Robots_[i].M[1]=fabs(sgn(Robots_[i].index_of_grabbing_posititon_+1))*sgn(Robots_[i].index_of_grabbing_posititon_+1);
//		Robots_[i].Dtau_=fabs(Robots_[i].M[1])*fabs(Robots_[i].M[0])*min(sgn(Robots_[i].M[0]),sgn(Robots_[i].M[1]))/((Object_.X_O_.block(0,0,3,1)-Vobject_.X_V_.block(0,0,3,1)).norm());
	...
	//	cout<<"handle_M "<<i<<" "<<Robots_[i].M[0]<<" "<<Robots_[i].M[1]<<" "<<Robots_[i].Dtau_<<" "<<Robots_[i].tau_<<" "<<Robots_[i].index_of_grabbing_posititon_<<" "<<Object_.Max_liklihood<<endl;
		//			Vobject_.Dgamma_=0.01/(Object_.X_O_INTERCEPT_.block(0,0,3,1).norm()+1e-32);
		/*		Robots_[i].tau_=Vobject_.gamma_;
		Robots_[i].Dtau_=Vobject_.Dgamma_;*/
		/*		Robots_[i].Dtau_=Robots_[i].Dtau_+Robots_[i].DDtau_*dt_;*/
...
/*		Robots_[i].tau_=.0;
		...
		/*		Robots_[i].tau_=1.0;
		Robots_[i].Dtau_=0.0;*/
		/*		if ((Vobject_.gamma_<=0.7)&&(Robots_[i].tau_==1.0))
		{
			Robots_[i].tau_=0.99;
		}*/
...
void task_allocation::calculate_coordination_parameter()
{
...
void task_allocation::calculate_robot_next_state()
{
	for (int i=0;i<N_robots_;i++)
	{
...
void task_allocation::calculate_ATX()
{
	for (int i=0;i<N_robots_;i++)
	{
...
void task_allocation::calculate_u()
{
...
void task_allocation::CalculateCatchingprobability(int nFrame,int i_robot,int i_object)
{
...
void task_allocation::predict_the_objects_position() // patrick
{
	...
	for(int i = 0; i < N_objects_; i++)
	{
		if ((Objects_[i].predict_->mReadyToPredict)&&(Objects_[i].X_O_(0)<4*minPos[0]))
		{
			...
			nFrame = Objects_[i].predict_->PredictNextPosVel(Objects_[i].MAX_PREDICTIONTIME_,dt_, Objects_[i].P_O_prediction_,true,0,maxPos[0]);
...
double task_allocation::robot_evaluate_task(int i_robot, int i_object)
{
	double value = 0;
	...
	else
	{
		...
		VectorXd delta_robot = Robots_[i_robot].X_Base_ - Objects_[i_object].X_O_;
		VectorXd delta_conveyor = Conveyor_end - Objects_[i_object].X_O_;
		value = 20.0*delta_robot.norm() + 200.0/delta_conveyor.norm();
	}
...
void task_allocation::predict_the_object_position()
{
	if ((Object_.predict_->mReadyToPredict)&&(Object_.X_O_(0)<4*minPos[0]))
	{
		int nFrame =0;
		nFrame = Object_.predict_->PredictNextPosVel(Object_.MAX_PREDICTIONTIME_,dt_, Object_.P_O_prediction_,true,0,maxPos[0]); // Patrick point of interest. Not too sure what "maxPos[0]" is, it is
...
void task_allocation::assign_the_robots()
{
	for (int i=0;i<N_robots_;i++)
	{
